...
public:
	Accumulator() : history(), lastUpdated(), leftProx(NULL), rightProx(NULL), aimer(NULL), encoderX(NULL), encoderY(NULL), pid(NULL), error(0), prevPredicted(0), lastLeftProxValue(0), lastRightProxValue(0), lastEncoderDistance(0), lastCameraAngle(0), errorX(0) {history.push_back(DoubleDouble(0,0,0)); lastUpdated.resize(0);}
	Accumulator(float x, float y, float z, int usNum, Ultrasonic& left, Ultrasonic& right, Aimer& aim, Encoder& encodx, Encoder& encody, PIDLoop& pd):
		history(), lastUpdated(), leftProx(&left),
		rightProx(&right), aimer(&aim), encoderX(&encodx), encoderY(&encody),
	...
	DoubleDouble driveToTower(bool hasBeenGoing, bool encoderPos, bool powerPos, bool goingPos, float target, float targetAngle, float targetError, float robotAngle);
