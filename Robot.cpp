...
void Robot::OperatorControl()
{
	...
	//ultrasonics
	float leftUltrasonic = leftProx.GetRangeInches(), leftUltrasonicComp = leftProx.GetRangeInches();
	float rightUltrasonic = rightProx.GetRangeInches(), rightUltrasonicComp = rightProx.GetRangeInches();
...
//		if(driveStick.GetRawButton(11) && !cameraSwitching){
//			cameraSwitching = true;
//
//			cameraInOperation = (cameraInOperation == 0)? 1 : 0;
//			camera = CameraServer::GetInstance()->StartAutomaticCapture(cameraInOperation);
//			SmartDashboard::PutString("Camera switch", std::to_string( cameraInOperation));
...
//		}
		...
		driveY = driveStick.GetRawAxis(Constants::driveYAxis);
		driveZ = driveStick.GetRawAxis(Constants::driveZAxis);
		driveX = fabs(driveX) < .1 ? 0.0 : driveX;
		driveY = fabs(driveY) < .1 ? 0.0 : driveY * .8;
		driveZ = fabs(driveZ) < .25 ? 0.0 : driveZ * .6;
